...
def add_locations(world, region, locations_dict):
    for (key, data) in locations_dict.items():
...
def assign_random_level_location(world, region, shop_locs, level_grouped_locs, shop_id, level):
    ...
    loc = shop_locs[shop_id].pop(random_key)
    create_location(world, region, random_key, loc.code)
    level_grouped_locs[shop_id][level] = loc.code
    return random_key, loc

def create_level_region(world, name: str, hint: str, shop_locs, level_grouped_locs, starting_region:bool = False):
    region = Region(name, world.player, world.multiworld)
    match = re.search(r'\d+', name)

    s1_key, s1_loc = assign_random_level_location(world, region, shop_locs, level_grouped_locs, 0, match.group(0))
    s2_key1, s2_loc1 = assign_random_level_location(world, region, shop_locs, level_grouped_locs, 1, match.group(0))
    s2_key2, s2_loc2 = assign_random_level_location(world, region, shop_locs, level_grouped_locs, 1, match.group(0))
    s3_key, s3_loc = assign_random_level_location(world, region, shop_locs, level_grouped_locs, 2, match.group(0))
    tt_key, tt_loc = assign_random_level_location(world, region, shop_locs, level_grouped_locs, 3, match.group(0))

    add_locations(world, region, locations.get_license_checks(world, s1_key, s1_loc, starting_region))
    ...
    if starting_region:
        world.starting_item_ids = [s1_loc.code, s2_loc1.code, s3_loc.code]

    add_locations(world, region, locations.get_level_checks(world, match.group(0)))
...
def create_regions(world):
    ...
    for l in range(0,world.options.max_level.value+5, 5):
        if l == 0:
            ...
            continue
        if l == 100:
            create_level_region(world, "Level 100", "Level 100", shop_locs, level_grouped_locs)
            continue
        create_level_region(world, f"Level {l}-{l+4}", f"Level {l}-{l+4}", shop_locs, level_grouped_locs)


    create_pack_region(world, CardRegion.BASIC, card_region_names[CardRegion.BASIC], min([level_grouped_locs[0][item_id] for item_id in [0,1] if item_id in level_grouped_locs[0]], default=None))
...
def connect_pack_region(world, card_region, level):
    if level is None:
        return
    end_level = level+4 if level != 1 else 5

    connect_regions(world, f"Level {level}-{end_level}", card_region_names[card_region], card_region_names[card_region])


def connect_entrances(world, level_grouped_locs):

    connect_pack_region(world, CardRegion.BASIC, min([level_grouped_locs[0][item_id] for item_id in [0,1] if item_id in level_grouped_locs[0]], default=None))
    connect_pack_region(world, CardRegion.RARE, min([level_grouped_locs[0][item_id] for item_id in [2,3] if item_id in level_grouped_locs[0]], default=None))
    connect_pack_region(world, CardRegion.EPIC, min([level_grouped_locs[0][item_id] for item_id in [4,5] if item_id in level_grouped_locs[0]], default=None))
    connect_pack_region(world, CardRegion.LEGENDARY, min([level_grouped_locs[0][item_id] for item_id in [6,7] if item_id in level_grouped_locs[0]], default=None))
    connect_pack_region(world, CardRegion.DESTINY_BASIC, min([level_grouped_locs[0][item_id] for item_id in [8,9] if item_id in level_grouped_locs[0]], default=None))
    connect_pack_region(world, CardRegion.DESTINY_RARE, min([level_grouped_locs[0][item_id] for item_id in [10,11] if item_id in level_grouped_locs[0]], default=None))
    connect_pack_region(world, CardRegion.DESTINY_EPIC, min([level_grouped_locs[0][item_id] for item_id in [12,13] if item_id in level_grouped_locs[0]], default=None))
    connect_pack_region(world, CardRegion.DESTINY_LEGENDARY, min([level_grouped_locs[0][item_id] for item_id in [14,15] if item_id in level_grouped_locs[0]], default=None))

    connect_regions(world, "Menu", "Level 1-4", "Level 1")
    connect_regions(world, "Menu", "Play Tables", "Play Table")
    connect_regions(world, "Menu", "Sell Tetramon", "Sell Tetramon")
    connect_regions(world, "Menu", "Sell Destiny", "Sell Destiny")
